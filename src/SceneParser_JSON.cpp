...
#include "SceneParser_JSON.hpp"

//
// function that the json parser will pick up to load vecs
//
namespace ES{
  void from_json(const json &j, ES::Vector3<float> &p) {
  ...
  void from_json(const json &j, ES::RGB &p) {
    // case when the input matches
    // "0.0 0.0 0.0",
    if (j.is_string()) {
      std::string s = j.get<std::string>();
...
void SceneParser_JSON::parseJSONData(const json &json_data) {
  ...
  if (json_context.contains("sceneParameters")) {
    ...
    sParams.bgColor =
        sceneParam.value("bgColor", ES::Vector3<float>{0.0f, 0.0f, 0.0f});
...
void SceneParser_JSON::parseCamera(const json &camera) {
  ES::Vector3<float> position, viewDir;
  position = camera["position"];

  if (camera.contains("lookatPoint")) {
    ES::Vector3<float> lookAtPoint = camera["lookatPoint"];
...
void SceneParser_JSON::parseLight(const json &light) {
  ...
  ES::Vector3<float> position; 
  ES::RGB radiantEnergy;
...
SceneParser_JSON::extractShaderProperty(const json &shaderParam,
  ...
  if (prop.is_object()) {
    if (prop.contains("data")) {
      ...
      result.data = prop["data"].get<ES::Vector3<float>>();
    } else {
    ...
    result.data = prop.get<ES::Vector3<float>>();
  }
...
void SceneParser_JSON::parseShader(const json &shader) {
  ...
  if (shaderDesc.type == "Lambertian" || shaderDesc.type == "Diffuse") {
  ...
  else if(shaderDesc.type == "white_chalk" || shaderDesc.type == "red_chalk" || shaderDesc.type == "blue_chalk" || shaderDesc.type == "yellow_chalk"){
      shaderDesc.diffuse = extractShaderProperty(shader, "diffuse");
  }
    ...
    float refrIdx;
    ES::Vector3<float> attenCoef;
    shaderDesc.attenuationCoef =
        shader.value("attenuationCoef", ES::Vector3<float>{1.0f, 1.0f, 1.0f});
...
void SceneParser_JSON::parseShape(const json &shape) {
  ...
  */
  ...
  if (shapeDesc.type == "sphere") {
    shapeDesc.center = shape.value("center", ES::Vector3<float>{0, 0, 0});
    shapeDesc.radius = shape.value("radius", 1.0f);
  }

  else if (shapeDesc.type == "triangle") {

    shapeDesc.v0 = shape.value("v0", ES::Vector3<float>{0, 0, 0});
    shapeDesc.v1 = shape.value("v1", ES::Vector3<float>{1, 0, 0});
    shapeDesc.v2 = shape.value("v2", ES::Vector3<float>{0, 1, 0});
  }

  else if (shapeDesc.type == "box") {
    shapeDesc.minPt = shape.value("minPt", ES::Vector3<float>{-1, -1, -1});
    shapeDesc.maxPt = shape.value("maxPt", ES::Vector3<float>{1, 1, 1});
  }
