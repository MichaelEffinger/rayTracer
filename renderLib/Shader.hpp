...
public:
    ...
    constexpr RGB operator()(const Hit& hit, const Ray<float>& ray, std::size_t depth, const Scene& scene) const noexcept;
};
...
public:
    ...
    RandomShader();
    RGB operator()(const Hit& hit, const Ray<float>& ray, std::size_t depth, const Scene& scene) const noexcept;
};
...
public:
    ...
    mutable std::mt19937 rng;
    mutable std::uniform_real_distribution<float> dist;
    DiffuseShader(RGB newColor) noexcept;
    RGB operator()(const Hit& hit, const Ray<float>& ray, std::size_t depth, const Scene& scene) const noexcept;
